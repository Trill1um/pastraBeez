...
export const signup = async (req, res) => {
    const { colonyName, email, password, messengerLink, confirmPassword } = req.body;
    try {
        ...
        if (!messengerLink || messengerLink.trim() === '') {
            return res.status(400).json({
                message: 'Messenger link is required - buyers need a way to contact you!'
            });
        }
        
        if (!messengerLink.includes('m.me/') && !messengerLink.includes('messenger.com/')) {
            return res.status(400).json({
                message: 'Please enter a valid Messenger link (m.me/username or messenger.com/t/username)'
            });
        }
        ...
        // Generate tokens and log user in immediately
        const {accessToken, refreshToken} = generateToken(user._id);
        await storeRefreshToken(user._id, refreshToken);
        setCookies(res, accessToken, refreshToken);
    
        res.status(201).json({ 
            message: "Colony registered successfully! Welcome to the hive!",
            user: {
                _id: user._id, 
...
export const refreshToken = async (req, res) => {
    console.log("refreshToken route activated");
    try {
